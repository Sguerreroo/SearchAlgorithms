...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
            print("Expanded nodes: " + str(expandNodes))
            return node
        if node.state not in closed:
            closed[node.state] = True
            fringe.extend(node.expand(problem))
            expandNodes = expandNodes + 1
    return None
...
def branch_bound_graph_search(problem):
    """Search the optimal path depending on the cost."""
    return graph_search(problem, ListByCost())


def branch_bound_with_heuristic_graph_search(problem):
    """Search the optimal path depending on the cost."""
    return graph_search(problem, CostHeuristic(problem))
